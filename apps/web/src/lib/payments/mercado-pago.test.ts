...
describe("Phase 11 Mercado Pago provider adapter", () => {
  it("exchanges an OAuth authorization code without leaking client credentials into the URL", async () => {
    ...
    expect(String(queue.requests[0]?.input)).not.toContain("phase11-client-secret");
  ...
  it("refreshes OAuth credentials and returns the rotated refresh token", async () => {
    const queue = createFetchQueue(
      jsonResponse({
  ...
  it("creates Checkout Pro preferences with split fee and durable references", async () => {
    const queue = createFetchQueue(
      jsonResponse({
        id: "pref-001",
  ...
  it.each([
    ...
    ["cancelled", "FAILED"],
    ["refunded", "REFUNDED"],
  ] as const)("normalizes provider payment status %s", async (rawStatus, expected) => {
    const queue = createFetchQueue(
      jsonResponse({
        id: 987654,
        status: rawStatus,
  ...
  it("rejects malformed authoritative provider payloads instead of guessing financial truth", async () => {
    const queue = createFetchQueue(
  ...
  it("supports partial and total refunds with provider idempotency keys", async () => {
    const queue = createFetchQueue(
      jsonResponse({ id: 1, payment_id: 987654, amount: 250, status: "approved" }),
      jsonResponse({ id: 2, payment_id: 987654, amount: 1000, status: "approved" }),
    );
    const provider = createProvider(queue.fetchImpl);
    ...
    expect(JSON.parse(String(queue.requests[0]?.init?.body))).toEqual({ amount: 250 });
  ...
  ] as const)("classifies transient provider HTTP %s", async (status, code) => {
    const queue = createFetchQueue(jsonResponse({ message: "temporary" }, status));
    const provider = createProvider(queue.fetchImpl);
  ...
  it("validates Mercado Pago webhook HMAC using the documented manifest and rejects tampering", () => {
