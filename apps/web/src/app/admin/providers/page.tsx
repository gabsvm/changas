...
export default async function AdminProvidersPage({
  ...
  return (
    <section className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Prestadores</h2>
        <p className="text-sm text-slate-600">
          Estado, catálogo, documentos y controles operativos.
        </p>
      </div>
      ...
      {detail ? (
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-600">
            {detail.email ?? "Sin email"} · {detail.service_count} servicios ·{" "}
            {detail.document_count} documentos
          </p>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <form action={setAccountRestrictionAction}>
              ...
              <input
                ...
                placeholder="Motivo"
              />
              <button className="mt-2 w-full rounded-lg bg-amber-600 px-3 py-2 text-sm font-semibold text-white">
                Restringir
              </button>
            </form>
            <form action={setAccountRestrictionAction}>
