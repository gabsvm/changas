...
export default async function HomePage() {
  ...
  return (
    <ConsumerShell maxWidth="max-w-7xl">
      <section className="grid gap-10 py-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:py-20">
        <div className="max-w-3xl">
          <p className="text-terracotta text-sm font-extrabold tracking-[0.12em] uppercase">
            Servicios reales, sin vueltas
          </p>
          <h1 className="font-display mt-5 text-5xl leading-[0.98] font-extrabold tracking-[-0.045em] sm:text-7xl">
            ¿Qué necesitás?
          </h1>
        ...
        <div className="brand-gradient-surface text-ink relative overflow-hidden rounded-[2.15rem] p-7 sm:p-9">
          ...
          <p className="text-ink/70 relative text-xs font-extrabold tracking-[0.12em] uppercase">
      ...
      >
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <h2
              id="categories-title"
              className="font-display mt-2 text-3xl font-extrabold tracking-[-0.03em]"
            >
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {(categories ?? []).map((category) => (
            <Link
              className="border-ink/10 bg-surface/85 hover:border-terracotta/25 hover:bg-surface rounded-2xl border p-5 shadow-[0_8px_24px_rgba(32,33,36,0.04)] transition hover:shadow-[0_12px_30px_rgba(255,107,53,0.10)]"
              href={"/categoria/" + category.slug}
              key={category.slug}
            >
              <h3 className="font-display text-2xl font-extrabold tracking-[-0.025em]">
                {category.name}
              </h3>
