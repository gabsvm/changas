-- Phase 05 hardening: keep Proposal/Job transitions provider-agnostic.
-- FakePaymentProvider remains a dev/test adapter; this RPC is the durable
-- server-only boundary that Phase 11 can reuse with a real payment provider.

alter table public.payment_attempts
  drop constraint if exists payment_attempts_provider_name_check;

alter table public.payment_attempts
  add constraint payment_attempts_provider_name_check
