-- Phase 03 audit hardening. Published migrations remain immutable.

alter table public.services
  add column if not exists search_text_normalized text generated always as (
    coalesce(public.normalize_search_text(title), '') || ' ' ||
    coalesce(public.normalize_search_text(description), '')
  ) stored;

drop index if exists public.services_search_text_trgm_idx;
create index services_search_text_trgm_idx
on public.services using gin (search_text_normalized extensions.gin_trgm_ops);

-- The original RPC remains in migration history but is no longer callable by
-- clients. The versioned contract below adds has_more and fixes its predicates.
  ...
  eligible as (
    ...
    left join lateral (
      ...
      where origin_point is not null
        ...
        and extensions.st_dwithin(a.center, origin_point, a.radius_meters)
    ) area on true
...
-- Keep the published RPC callable for older clients, but route it through the
-- corrected read model. New clients use v2 so they can render has_more.
...
drop policy if exists provider_favorites_insert_own on public.provider_favorites;
drop policy if exists provider_favorites_delete_own on public.provider_favorites;
revoke insert, update, delete on table public.provider_favorites from authenticated;
grant select on table public.provider_favorites to authenticated;
